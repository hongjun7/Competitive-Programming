...
struct MinCostFlow {
	...
	MinCostFlow(int n) : G(n), n(n) {}

	void addEdge(int s, int e, cost_t cost, cap_t cap) {
		if (s == e) return;
		edge forward{ e, cost, cap, cap, G[e].size() };
		edge backward{ s, -cost, 0, 0, G[s].size() };
		G[s].emplace_back(forward);
		G[e].emplace_back(backward);
	}
	...
	pair<cost_t, cap_t> augmentShortest(int s, int e, cap_t flow_limit) {
		...
		while (!q.empty()) {
			...
			for (const auto& e : G[cur]) {
				if (iszerocap(e.residual)) continue;
				auto next = e.target;
				auto ncost = dist[cur].first + e.cost;
				auto nflow = min(dist[cur].second, e.residual);
		...
		while (from[p] != -1) {
			auto nedge = from[p];
			auto np = G[p][nedge].target;
			auto fedge = G[p][nedge].revid;
			G[p][nedge].residual += flow;
			G[np][fedge].residual -= flow;
			p = np;
		}
